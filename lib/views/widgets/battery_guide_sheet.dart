...
const _romGuides = <_RomGuide>[
  _RomGuide(
    '小米 / 红米 (MIUI)',
    '设置 → 应用设置 → 应用管理 → 轻舟 → 省电策略 → 无限制；并在 自启动管理 中允许轻舟自启动',
  ),
  _RomGuide(
    '华为 / 荣耀 (EMUI)',
    '设置 → 电池 → 更多电池设置 关闭"休眠时始终保持网络连接"后，进入 应用启动管理 → 轻舟 → 关闭自动管理，手动开启全部允许',
  ),
  _RomGuide(
    'OPPO / 一加 (ColorOS)',
    '设置 → 电池 → 应用耗电管理 → 轻舟 → 允许完全后台行为 / 关闭智能后台管控',
  ),
  _RomGuide(
    'vivo / iQOO (OriginOS)',
    '设置 → 电池 → 后台功耗管理 → 轻舟 → 允许后台高耗电；设置 → 应用 → 轻舟 → 自启动开启',
  ),
